...
const pageTextTargets = [
  ...
  { key: "metrics.games", label: "统计：训练工具", selector: ".metric-strip .metric:nth-child(3) span", short: true },
  ...
  { key: "metrics.gamesNote", label: "统计说明：训练工具", selector: ".metric-strip .metric:nth-child(3) .metric-note", short: true },
...
function renderStats() {
  ...
  nodes.statGames.textContent = apps.filter(app => app.status === "game" || app.status === "training").length;
}
...
function renderGameGrid(filtered) {
  ...
  const gameList = filtered.filter(app => app.status === "game" || app.status === "training");
  if (nodes.gameCount) {
    nodes.gameCount.textContent = `${gameList.length} 个训练工具`;
  }
  if (!gameList.length) {
