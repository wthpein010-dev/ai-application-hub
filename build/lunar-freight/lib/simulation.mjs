...
export class LunarSimulation {
 constructor(){
  ...
  this.cargo=BASES.map((b,i)=>({id:b.id,name:b.cargo,state:'carried',offset:new C.Vec3((i-1)*.65,.92,.72),body:null}));
 ...
 get complete(){return this.delivered===3;}
 ...
 impact(speed){
  ...
  this.lastImpact=this.time;this.impacts++;c.state='dropped';this.body.removeShape(c.shape);
 ...
 interact(){
  ...
  for(const c of this.cargo)if(c.state==='dropped'&&this.body.position.distanceTo(c.body.position)<6){this.world.removeBody(c.body);c.body=null;c.state='carried';this.body.addShape(c.shape,c.offset);this.say(`${c.id} 箱设备已重新装车。`);return true;}
  for(const b of BASES)if(Math.hypot(this.body.position.x-b.x,this.body.position.z-b.z)<6&&Math.abs(this.body.position.y-heightAt(b.x,b.z))<2.5&&this.grounded>=2){const c=this.cargo.find(c=>c.id===b.id);if(c.state==='carried'){c.state='delivered';this.body.removeShape(c.shape);this.battery=Math.min(100,this.battery+25);this.say(`${b.name}交付成功，电量补充 25%。`);return true;}}
 ...
 snapshot(){return {x:this.body.position.x,y:this.body.position.y,z:this.body.position.z,heading:this.heading,speed:this.speed,grounded:this.grounded,upright:this.upright,battery:this.battery,time:this.time,delivered:this.delivered,complete:this.complete,impacts:this.impacts,resets:this.resets,message:this.messageUntil>this.time?this.message:'',cargo:this.cargo.map(c=>({id:c.id,name:c.name,state:c.state,x:c.body?.position.x,z:c.body?.position.z}))};}
}
