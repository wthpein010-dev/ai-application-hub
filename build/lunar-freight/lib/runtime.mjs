...
export function createRuntime(canvas,publish,onError){
 let sim=new LunarSimulation(),view=createScene(canvas,sim),started=false,paused=false,mode=2,raf=0,last=performance.now(),acc=0,ui=0,external=null,disposed=false;
 const keys=new Set(),lifecycle=new AbortController();
 const snapshot=()=>({...sim.snapshot(),started,paused,mode});
 const clear=()=>{keys.clear();external=null;};
 function action(name,value){
  if(name==='start'){started=true;paused=false;}
  ...
  else if(name==='restart'){clear();view.dispose();sim=new LunarSimulation();view=createScene(canvas,sim);started=true;paused=false;acc=0;}
  else if(started&&!paused){if(name==='interact')sim.interact();else if(name==='reset')sim.reset();}
  publish(snapshot());canvas.focus({preventScroll:true});return snapshot();
 }
 const down=e=>{if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code))e.preventDefault();if(e.repeat)return;if(e.code==='Enter'&&!started)action('start');if(e.code==='Escape'){action('pause');return;}if(e.code.startsWith('Digit')&&['1','2','3'].includes(e.key))action('camera',Number(e.key));if(e.code==='KeyC')action('camera',mode%3+1);if(e.code==='KeyE')action('interact');if(e.code==='KeyR')action('reset');keys.add(e.code);};
 ...
 function frame(now){
  ...
  if(started&&!paused){acc+=dt;while(acc>=1/120){const input=external||{throttle:keys.has('KeyW')||keys.has('ArrowUp')?1:keys.has('KeyS')||keys.has('ArrowDown')?-1:0,steer:keys.has('KeyA')||keys.has('ArrowLeft')?1:keys.has('KeyD')||keys.has('ArrowRight')?-1:0,brake:keys.has('Space'),solar:keys.has('KeyT')};sim.step(1/120,input);acc-=1/120;}}else acc=0;
 ...
 if(context?.registerTool){
  ...
  register({name:'mission_status',description:'读取月球车位置、航向、速度、电池和三箱设备的任务状态。',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:snapshot});
  register({name:'rover_action',description:'开始、暂停/继续、重开、装卸、扶正或切换镜头。遵循与页面按钮相同的规则。',inputSchema:{type:'object',properties:{action:{type:'string',enum:['start','pause','restart','interact','reset','camera']},camera:{type:'integer',enum:[1,2,3]}},required:['action'],additionalProperties:false},execute:({action:name,camera})=>{if(!['start','pause','restart','interact','reset','camera'].includes(name))throw Error('无效动作');return action(name,camera);}});
  register({name:'drive_rover',description:'持续操作当前月球车，经过真实时间和同一物理循环后返回状态。不会传送车辆或直接交付。油门 -1 到 1，转向正数左转，负数右转。',inputSchema:{type:'object',properties:{throttle:{type:'number',minimum:-1,maximum:1},steer:{type:'number',minimum:-1,maximum:1},brake:{type:'boolean'},seconds:{type:'number',minimum:.1,maximum:8}},required:['throttle','steer','seconds'],additionalProperties:false},execute:async input=>{
   if(!started||paused||sim.complete)throw Error('请先开始或继续未完成的任务');
   ...
   await new Promise(resolve=>{const check=()=>{if(disposed||paused||sim.complete||sim.time>=target||performance.now()>deadline){external=null;resolve();}else setTimeout(check,30);};check();});publish(snapshot());return snapshot();
  }});
 }
