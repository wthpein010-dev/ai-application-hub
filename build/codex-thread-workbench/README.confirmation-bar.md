...
面向 Windows 和 macOS 的 Codex 待确认提醒工具。它会扫描需要你决定是否继续的 Codex 任务；默认停靠在最左侧屏幕的左侧中部，空闲时收纳为窄边签，避开浏览器顶部。鼠标停留片刻才展开，快速经过不会误弹；发现任务后自动展开，让你查看原任务、忽略误报或确认继续。

## Windows 使用

展开后采用窄竖版任务栏：顶部状态与自动开关、中间滚动任务卡片、底部固定批量确认。窗口高度会适应目标屏幕，任务增减时保持尺寸稳定；长错误说明不会挤走底部操作。
