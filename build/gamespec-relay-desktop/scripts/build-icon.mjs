...
try {
  ...
  await page.setContent(`<!doctype html><html><style>
    *{box-sizing:border-box}html,body{margin:0;width:512px;height:512px;overflow:hidden}
    body{display:grid;place-items:center;background:#07110f;font-family:Arial,sans-serif}
    ...
    .letters{position:absolute;left:54px;bottom:72px;color:#eff8f4;font-size:170px;font-weight:900;letter-spacing:-18px;line-height:.8;text-shadow:0 12px 30px rgba(0,0,0,.35)}
    .label{position:absolute;right:50px;bottom:54px;color:#5de2b3;font-size:26px;font-weight:800;letter-spacing:7px}
  </style><body><div class="icon"><div class="grid"></div><div class="relay"></div><div class="letters">GR</div><div class="label">RELAY</div></div></body></html>`);
  await page.screenshot({ path: output, type: "png" });
} finally {
  await browser.close();
}

console.log(`GameSpec Relay icon generated: ${output}`);
