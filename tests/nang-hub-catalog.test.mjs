...
test("Nang WebGL build does not depend on server compression headers", () => {
  const projectRoot = join(root, "projects", "nang-keng-pai-pai-xiang");
  const preview = readFileSync(join(projectRoot, "index.html"), "utf8");

  assert.match(preview, /WebGL\.data/);
  assert.match(preview, /WebGL\.framework\.js/);
  assert.match(preview, /WebGL\.wasm/);
  assert.doesNotMatch(preview, /\.unityweb/);
  assert.equal(existsSync(join(projectRoot, "Build", "WebGL.data")), true);
  assert.equal(existsSync(join(projectRoot, "Build", "WebGL.framework.js")), true);
  assert.equal(existsSync(join(projectRoot, "Build", "WebGL.wasm")), true);
});
