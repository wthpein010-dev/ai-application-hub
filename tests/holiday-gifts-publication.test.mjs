...
test('holiday prototype is in project support with real demo and video, without fake platform downloads',()=>{
 const apps=loadDefaultAppsFromRuntime(read('app-20260706-restore-games.js'));
 const app=apps.filter(a=>a.status==='engineering').at(-1);assert.equal(app.id,'holiday-gifts');assert.equal(app.badge,'项目辅助');
...
test('demo and video reuse shared shell and local playable resources',()=>{
 const html=read('projects/holiday-gifts/index.html');assert.match(html,/<title>好事成双 · 双节活动<\/title>/);assert.match(html,/subpage-shell.css/);assert.match(html,/index.html#overview/);assert.match(html,/game\/index.html/);
