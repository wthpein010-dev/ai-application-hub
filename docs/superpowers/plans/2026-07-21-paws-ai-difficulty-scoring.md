...
- [x] 写已有/可解/不可解关卡的评分失败测试，覆盖五维权重、评级和发布门禁。
- [x] 扩展统计与求解轨迹，提供开放率、平均遮挡、阶段对子、伙伴距离、有效分支和容错代理。
- [x] 实现分段锚点评分、维度重加权、总分、可信度和主要原因。
- [x] 运行核心测试并保持既有求解测试通过。
...
- [x] 写 180/12/40、200/15/60、240/32/80 与奇数规范化的失败测试。
- [x] 将难度档改为默认值/建议范围，支持 `tileCount`、`layerCount`、`targetScore` 和最多 64 个候选。
- [x] 让生成结果保存实际评分、目标偏差和候选数，并拒绝不可解候选。
- [x] 验证固定种子可复现、不同目标分产生可解释差异。
...
- [x] 写阶段砖量、有效层、平台拆分、反重叠、开局安全对子和释放段的失败测试。
- [x] 实现五阶段偶数配额、多局部塔、小平台缺口与阶段压力元数据。
- [x] 保证每个请求层都真实参与依赖，且平均遮挡和同格堆叠不过线。
- [x] 对 200/15 和 240/32 做性能与可解性回归。
...
- [x] 写三个精确输入、切档自动填充、建议文案和生成摘要的失败测试。
- [x] 增加砖块数、有效层数、目标难度，保持主选择数量不变。
- [x] 规范化偶数和边界，控制器透传参数并展示五维结果。
- [x] 验证已有保存、重名、导入和只读模式不回归。
...
- [x] 浏览器生成标准 200/15/60，验证实际数量、评分、保存、2D/3D、试玩和刷新恢复。
- [x] 执行全部 Paws 单元、契约、静态服务器和浏览器测试，检查语法与 `git diff --check`。
- [x] 原始分辨率检查对话框和 3D 截图，无裁切、遮挡或重叠文本。
- [x] 提交并推送 `origin/main`，等待 Pages 成功后做线上 HTTP 与真实浏览器验收。
