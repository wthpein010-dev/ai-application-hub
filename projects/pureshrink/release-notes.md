...
- 浏览器在加载期取消时会在核心加载结束后再次退出，转换输出复制后也会先清空虚拟文件并释放转换核心，再启动无损验证核心。
- 桌面端任一无损指纹失败或用户取消时会中止同组验证，并等待输入、输出两个 FFmpeg 作业都结束后再清理输出和任务状态。
- Hub 会把 PureShrink 的缓存动作入口迁移到当前公开默认值，同时保留用户自定义名称和文案。

## 1.0.4 稳定性优化

- 浏览器每次媒体处理和无损指纹验证后都会主动退出 FFmpeg WebAssembly 核心；加载失败、运行失败与取消路径同样释放资源，连续批量处理不再累积 Worker 和共享内存。
- 桌面端在任务已取消时不再启动原生进程，并使处理阶段与无损验证阶段共用同一取消信号。
- 原生 FFmpeg 的标准输出和错误输出分别只保留最新 1 MiB 诊断尾部，避免长视频处理时日志无界增长。
- 新增成功、加载失败、预取消、日志边界和指纹取消回归测试。
