import { assembleDownload, sha256Hex, validateManifest } from "../download-core.js";

const elements = {
  architectures: [...document.querySelectorAll('[data-role="architecture"]')],
  selectedArchitecture: document.querySelector('[data-role="selected-architecture"]'),
...
const labels = { arm64: "Apple 芯片", x64: "Intel" };
let selectedButton = elements.architectures[0];
let manifest;
let running = false;
let manifestRequest = 0;

const formatBytes = (bytes) => new Intl.NumberFormat("zh-CN", {
...
const renderProgress = (event) => {
  const percent = Math.min(100, Math.round((event.loadedBytes / manifest.totalSize) * 100));
  elements.progress.value = percent;
  elements.progressText.textContent = `${formatBytes(event.loadedBytes)} / ${formatBytes(manifest.totalSize)} · ${percent}%`;
  if (event.phase === "part-start") elements.status.textContent = `正在下载第 ${event.partIndex + 1} / ${manifest.parts.length} 个分片`;
  if (event.phase === "retry") elements.status.textContent = `第 ${event.partIndex + 1} 个分片失败，正在重试（${event.attempt + 1} / 3）`;
  if (event.phase === "verifying") elements.status.textContent = "全部分片已下载，正在校验完整 App ZIP…";
};
...
const loadManifest = async () => {
  ...
  try {
    const response = await fetch(selectedButton.dataset.manifest, { cache: "no-store" });
    if (!response.ok) throw new Error(`下载清单读取失败：HTTP ${response.status}`);
    ...
    if (nextManifest.fileName !== expectedFileName) throw new Error(`下载清单文件名与 ${architecture} 架构不匹配。`);
...
const selectArchitecture = (button) => {
  ...
  const architecture = button.dataset.architecture;
...
const startDownload = async () => {
  ...
  running = true;
  elements.architectures.forEach((button) => { button.disabled = true; });
  elements.downloadButton.disabled = true;
  clearError();
  try {
    ...
    running = false;
    elements.architectures.forEach((button) => { button.disabled = false; });
    elements.downloadButton.disabled = !manifest;
  }
};
elements.architectures.forEach((button) => button.addEventListener("click", () => selectArchitecture(button)));
elements.downloadButton.addEventListener("click", startDownload);
elements.retryButton.addEventListener("click", () => manifest ? startDownload() : loadManifest());
loadManifest();
