...
page.on("requestfailed", (request) => {
  ...
  if (request.url().startsWith("blob:") && failure.includes("ERR_ABORTED")) {
    localBlobAborts.push(`${request.url()} ${failure}`);
    return;
  }
...
try {
  ...
  const externalOpens = await page.evaluate(() => window.__tutorialExternalOpens);
  ...
  await waitUntil(startedAt, 39);
  await scrollTo(page, "#comparison-result", 112);
  assert.notEqual((await page.locator("#similarity-class").textContent()).trim(), "等待候选");

  await waitUntil(startedAt, 47);
  await scrollTo(page, "#license-ledger");
  await page.locator("#license-source").selectOption("OpenGameArt");
  await page.locator("#license-url").fill("https://opengameart.org/content/original-synthetic-demo");
  await page.locator("#license-name").fill("CC0");
  await page.locator("#license-hash").fill("0".repeat(64));
  await page.locator("#license-author").fill("原创合成演示");
  ...
  assert.match(await page.locator("#license-list .license-entry").textContent(), /CC0/);
  ...
  await waitUntil(startedAt, durationSeconds);
  ...
  await writeFile(metadataPath, `${JSON.stringify({ durationSeconds, externalOpenCount: externalOpens.length }, null, 2)}\n`, "utf8");
  ...
  process.stdout.write([
    ...
    `Expected local blob aborts: ${localBlobAborts.length}`,
    `Unmatched requests: ${unmatchedRequests.length}`,
    `External opens intercepted: ${externalOpens.length}`,
  ].join("\n") + "\n");
} finally {
  if (context.pages().length) await context.close().catch(() => undefined);
