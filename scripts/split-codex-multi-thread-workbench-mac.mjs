...
const DEFAULT_CHUNK_SIZE = 8_388_608;
const ARCHITECTURES = new Set(["arm64", "x64"]);
const sha256 = (bytes) => createHash("sha256").update(bytes).digest("hex").toUpperCase();
const [sourceArgument, outputArgument, architecture, chunkSizeArgument] = process.argv.slice(2);

if (!sourceArgument || !outputArgument || !ARCHITECTURES.has(architecture)) {
  console.error("Usage: node scripts/split-codex-multi-thread-workbench-mac.mjs <source-app-zip> <download-directory> <arm64|x64> [chunk-size]");
  process.exitCode = 64;
} else {
  try {
    const chunkSize = chunkSizeArgument === undefined ? DEFAULT_CHUNK_SIZE : Number(chunkSizeArgument);
    if (!Number.isSafeInteger(chunkSize) || chunkSize <= 0) throw new Error("Chunk size must be a positive integer.");
    ...
    for (let offset = 0, index = 0; offset < source.byteLength; index += 1) {
      const bytes = source.subarray(offset, Math.min(offset + chunkSize, source.byteLength));
