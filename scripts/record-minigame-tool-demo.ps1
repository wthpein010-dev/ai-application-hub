...
$videoDir = Join-Path $RepositoryRoot 'projects\minigame-project-tool\video'
$frameDir = Join-Path $videoDir 'frames'
$timeline = Get-Content -Raw -Encoding UTF8 (Join-Path $RepositoryRoot 'scripts\minigame-tool-video-script.json') | ConvertFrom-Json
$output = Join-Path $videoDir 'minigame-project-tool-intro.mp4'
$caption = Join-Path $videoDir 'minigame-project-tool-intro.vtt'
$temp = Join-Path ([IO.Path]::GetTempPath()) ('minigame-tool-video-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    for ($i = 0; $i -lt $timeline.Count; $i++) {
        ...
        $filter = "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=0x07101e,zoompan=z='min(zoom+0.00012,1.025)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${frames}:s=1920x1080:fps=30,format=yuv420p"
    ...
    try {
        & $ffmpeg -y -loglevel error -i $joinedPath -vf "subtitles=minigame-project-tool-intro.vtt:force_style='FontName=Microsoft YaHei,FontSize=18,PrimaryColour=&H00FFFFFF,OutlineColour=&H90000000,BorderStyle=3,Outline=1,Shadow=0,MarginV=48,Alignment=2'" -c:v libx264 -preset medium -crf 21 -c:a copy -movflags +faststart $output
